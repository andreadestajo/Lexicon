    ...
    <div>
      <q-card class="my-card dashboard-cards">
        <q-card-section class="text-center">
          <div class="text-grey font-raleway" style="font-size: 18px">
            <b>
              <i
                class="fas fa-wave-square q-mr-md q-pa-sm"
                style="font-size: 25px; border-radius: 50%; background-color: #2f4c7e; color: #fff"
              ></i>
              PERFORMANCE FEE
            </b>
          </div>
          <p style="font-size: 50px; transition-delay: 2s" class="font-bebas">
            <b v-if=" this.default_data[5].charge_type == 'Percentage' ">
              {{ this.default_data[5].value == 0 ? 'No fee' : this.default_data[5].value }}
              <span
                v-if=" this.default_data[5].value != 0 "
              >{{ this.default_data[5].charge_type == 'Percentage' ? '%' : '$'}}</span>
            </b>

            <b v-if=" this.default_data[5].charge_type == 'Fixed' ">
              <span
                v-if=" this.default_data[5].value != 0 "
              >{{ this.default_data[5].charge_type == 'Percentage' ? '%' : '$'}}</span>
              {{ this.default_data[5].value == 0 ? 'No fee' : this.default_data[5].value }}
            ...
            <b v-if=" this.default_data[5].value == 0">
              <span
                v-if=" this.default_data[5].value != 0 "
              >{{ this.default_data[5].charge_type == 'Percentage' ? '%' : '$'}}</span>
              {{ this.default_data[5].value == 0 ? 'No fee' : this.default_data[5].value }}
