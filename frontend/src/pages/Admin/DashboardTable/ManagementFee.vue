    ...
    <div>
      <q-card class="my-card dashboard-cards">
        <q-card-section class="text-center">
          ...
          <p id="p-font-management-fee" class="font-bebas">
            <b v-if=" this.default_data[4].charge_type == 'Percentage' ">
              {{ this.default_data[4].value == 0 ? 'No fee' : this.default_data[4].value }}
              <span
                v-if=" this.default_data[4].value != 0 "
              >{{ this.default_data[4].charge_type == 'Percentage' ? '%' : '$'}}</span>
            </b>

            <b v-if=" this.default_data[4].charge_type == 'Fixed' ">
              <span
                v-if=" this.default_data[4].value != 0 "
              >{{ this.default_data[4].charge_type == 'Percentage' ? '%' : '$'}}</span>
              {{ this.default_data[4].value == 0 ? 'No fee' : this.default_data[4].value }}
            ...
            <b v-if=" this.default_data[4].value == 0">
              <span
                v-if=" this.default_data[4].value != 0 "
              >{{ this.default_data[4].charge_type == 'Percentage' ? '%' : '$'}}</span>
              {{ this.default_data[4].value == 0 ? 'No fee' : this.default_data[4].value }}
